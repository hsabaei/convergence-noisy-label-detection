...
def parse_args():
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/ckl_le_fusion_noise05_k40"),
    )
...
def auc_binary(y, score):
    ...
    if np.sum(valid & y) == 0 or np.sum(valid & ~y) == 0:
        return np.nan
...
def main():
    ...
    print("=" * 72)
    print("CKL + LE-GIE FUSION EXPERIMENT")
    print("=" * 72)
    print(f"K={K}")
    ...
    print("Computing LE-GIE next/next K=40 ...")
    ...
    for w in args.weights:
        # Do not duplicate the named pure endpoints.
    ...
    # Remove exact duplicate 50/50 key if created above.
    ...
    write_csv(
        args.output_dir / "fusion_score_auc_summary.csv",
        score_auc_rows,
    )

    # ============================================================
    # 2) Complementarity of CKL vs LE
    # ============================================================
    complementarity_rows = []

    # Evaluate overlap every monitored epoch.
    ...
    write_csv(
        args.output_dir / "ckl_le_complementarity_by_epoch.csv",
        complementarity_rows,
    )

    # ============================================================
    # 3) Apply same rank-based temporal detectors to each variant.
    # ============================================================
    detector_rows = []
    detector_auc_rows = []
    ...
    for name, score_nt in variants.items():
        print()
        print(f"Evaluating temporal detectors: {name}")
        ...
        # Continuous run length for ROC-AUC.
        run_stat = np.full(
            hit_nt.shape,
            np.nan,
            dtype=np.float32,
        ...
        row, _ = detector_stat_auc_summary(
            name,
            run_stat,
            y,
            epochs,
        ...
        detector_auc_rows.append({
            "variant": name,
            "detector": "min_run_stat",
            "best_stat_auc": row["best_auc"],
            "best_stat_auc_epoch": row["best_epoch"],
        })
    ...
    write_csv(
        args.output_dir / "fusion_detector_primary_summary.csv",
        detector_rows,
    )
    write_csv(
        args.output_dir / "fusion_detector_stat_auc_summary.csv",
        detector_auc_rows,
    )

    # ============================================================
    # 4) Compact "does fusion beat both?" summary
    # ============================================================
    decision_rows = []

    # Score-level endpoints.
    base_score = {
        r["variant"]: r
        for r in score_auc_rows
    }

    ckl_auc = base_score["CKL_rank"]["best_auc"]
    le_auc = base_score["LE_rank"]["best_auc"]
    best_single_auc = max(ckl_auc, le_auc)
    ...
    for r in score_auc_rows:
        ...
        decision_rows.append({
            ...
            "metric": "best_auc",
            "value": r["best_auc"],
            "best_single_reference": best_single_auc,
            "improvement_over_best_single": (
                r["best_auc"] - best_single_auc
                if is_fusion
                else np.nan
            ),
            "beats_best_single": (
                bool(r["best_auc"] > best_single_auc)
                if is_fusion
                else False
            ),
        })

    # Detector-statistic AUC endpoints.
    by_detector = {}
    ...
    write_csv(
        args.output_dir / "fusion_improvement_summary.csv",
        decision_rows,
    )
    ...
    config = {
        "artifact": "CKL_LE_score_fusion_noise05_K40",
        ...
        "evaluation_note":
            "synthetic anomaly mask is used only for ROC-AUC/TPR/FPR evaluation, never for fusion",
        "selection_note":
            "weight sweep is exploratory on the labeled 5% run; freeze a selected fusion rule before testing another seed/noise level",
    }

    with (
        args.output_dir / "fusion_config.json"
    ).open(
        "w",
        encoding="utf-8",
    ) as f:
    ...
    print()
    print("=" * 72)
    print("PRIMARY FIXED-q DETECTOR RESULTS")
    print("=" * 72)
    for r in detector_rows:
        print(
            f"{r['variant']:>18s} | "
